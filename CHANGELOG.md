...
- **`X-Forwarded-*`-Header waren ungeprüft vertrauenswürdig — Host-Header-Injection und faelschbare Audit-IP.** Bei der Prüfung, ob die App mit segmentierten Unternehmensnetzen (DMZ/DB-Netz/Server-Netz, ggf. mit intern genutzten offiziellen IP-Bereichen) zurechtkommt, stellte sich heraus: Die DB-Anbindung selbst ist unkritisch (reines TCP über `DB_HOST:DB_PORT`, keine IP-Range-Annahmen im Code), aber zwei Stellen lasen `X-Forwarded-For`/`-Host`/`-Proto` direkt und ungeprüft — ein Client konnte diese Header selbst mitschicken, unabhängig davon, ob überhaupt ein Reverse-Proxy davorstand:
  ...
  Die übrigen Aspekte der Netzsegmentierungs-Frage (DB-Konnektivität, Verschlüsselung über Segmentgrenzen, was ungetestet blieb) sind mit Status dokumentiert in `docs/architecture/c4-diagrams.md` → „Network segmentation".
...
- **i18n-Nachzug, Teil 11: NIS2 (Art. 21-Checkliste & Art. 23-Meldefristen).** Governance-Checkliste (Tabelle, Filter, Formular) und Meldefristen-Tracking (Übersicht, Vorfall-Formular mit Phasen) waren fuer FR/NL durchgehend englisch. 7 neue Schluessel, Rest wiederverwendet (`ui_back`, `ui_cancel`, `ui_save`, `ui_priority`, `col_title`, `col_status`, `ui_searchPh`). Nebenbei gefunden: eine mitten im Text eingebettete deutsche Restzeit-Angabe ("noch X h") war unabhaengig von der Sprache fest verdrahtet — jetzt als Parameter-Schluessel `nis2_hoursLeft`. Beim Durchsuchen des Umfelds zwei weitere Fremdfunde aus bereits abgeschlossenen Modulen mitgenommen: das Governance-Modul (Teil 3) hatte noch ein rohes `Error: ...` in `switchGovTab` und ein hartcodiertes "All sources" im Aktionen-Quellenfilter; beide jetzt ebenfalls uebersetzt.
- **i18n-Nachzug, Teil 10: Training & modulübergreifender Links-Picker.** Schulungsübersicht (KPI-Karten, Fälligkeitsanzeige), Schulungsplan (Tabelle, "Neue Schulung"-Button) und das komplette Schulungsformular waren fuer FR/NL durchgehend englisch. Rund 20 neue Schluessel. Nebenbei gefunden — **groesserer Fund:** `renderLinksBlock`, der SoA-Controls/Policies-Verknuepfungs-Picker, der in praktisch jedem Formular quer durchs ganze Tool eingebunden ist (Training, Governance, Legal, GDPR, Risk, BCM, Findings u.a.), war komplett fest verdrahtet (Mix aus Deutsch und Englisch: "Links", "SoA-Controls", "Suche…", "Policies / Templates", "Double-click to add") — unabhaengig von der Spracheinstellung, in allen bereits abgeschlossenen Modulen ebenso betroffen. Jetzt ueber 6 neue `link_*`-Schluessel uebersetzt; wirkt rueckwirkend auf alle Formulare, die diesen Picker einbinden. Ausserdem `training_noEvidence`-Nachbarbug behoben: der Platzhaltertext "Kein Nachweis hinterlegt." in der Nachweise-Ansicht war unabhaengig von der Sprache fest auf Deutsch verdrahtet.
- **i18n-Nachzug, Teil 9: Berichte.** Die Berichtsergebnis-Renderer (`renderReportResult`) fuer alle neun Berichtstypen (Compliance, Framework-Abdeckung, Gap-Analyse, Template-Uebersicht, Faellige Reviews, Compliance-Matrix, Audit-Trail, Audit-Feststellungen, Risikoregister) waren fuer FR/NL durchgehend englisch — Tabellenkoepfe, KPI-Labels und Statuswerte, die direkt aus Rohdaten gerendert wurden statt ueber `t()`. Rund 7 neue Schluessel, der Rest wiederverwendet (`riskl_*`, `status_draft/review/approved/archived`, `findings_*`, `reports_days`, `ui_score`, `soa_notApplicable`). Nebenbei gefunden: `FINDING_SEVERITY_LABELS`, `FINDING_STATUS_LABELS` und `FINDING_ACT_STATUS_LABELS` waren als einfache Konstanten mit `t()`-Werten zur Ladezeit eingefroren statt live nachzuziehen — nach dem Getter-Muster (RISK_STATUSES) umgestellt, betrifft auch die Feststellungs-Badges ausserhalb der Berichte. Ausserdem eine Variablenverschattung behoben: `reviewRow`s Parameter hiess `t` und ueberschattete die globale `t()`-Funktion — umbenannt zu `row`.
- **i18n-Nachzug, Teil 8: Admin & Organisation.** Organisationseinstellungen-Tab (IT-Organisationseinheiten-Tabelle), OE-Formular und Benutzer-Formular (Rollen- und Funktionsdropdowns, Passwort-Platzhalter) waren fuer FR/NL durchgehend englisch. Rund 20 neue Schluessel in DE/EN/FR/NL. Nebenbei gefunden: `ROLES_LIST` und `FUNCTIONS_LIST` (die acht RBAC-Rollen- bzw. sieben Organisationsfunktions-Beschreibungen, u.a. im Benutzer-Formular und in den Funktions-Badges der Benutzertabelle) sowie die OE-Typbezeichnungen (CIO/Group/Local/External) waren fest auf Englisch verdrahtet, unabhaengig von der Spracheinstellung; Rollen-/Funktionslisten jetzt ueber Getter wie die uebrigen modul-uebergreifenden Konstanten.
- **i18n-Nachzug, Teil 7: Risiko & Behandlung.** Übersichtsseite, Risikoregister, Kalender, Berichte, Detailansicht, Risiko-Formular und Maßnahmen-Modal waren fuer FR/NL durchgehend englisch — sieben Funktionen. Rund 45 neue Schluessel in DE/EN/FR/NL. Nebenbei gefunden: `RISK_STATUSES` (Open/In Treatment/Accepted/Closed, der Status des Risikos selbst — nicht der Maßnahme) war fest auf Englisch verdrahtet, unabhaengig von der Spracheinstellung; jetzt ueber Getter wie die uebrigen Risiko-Konstanten. Die „Verknüpfte Policies"-Überschrift in der Detailansicht war ebenso sprachunabhaengig fest auf Deutsch verdrahtet.
- **i18n-Nachzug, Teil 6: BCM (Notfallmanagement).** BIA-Formular und -Register, Notfallpläne (inkl. „Überfällig"-Hinweis) und Übungen/Tests waren fuer FR/NL durchgehend englisch — sechs Funktionen insgesamt. Rund 60 neue Schluessel in DE/EN/FR/NL, wiederverwendet u.a. `assets_criticality`, `col_status`, `col_title`, `col_type`, `ui_actions`, `ui_date`, `ui_entity`, `ui_cancel`, `ui_save`, `ui_docsAttachments`.
- **i18n-Nachzug, Teil 5: Assets & Lieferanten.** Das Asset-Formular sowie alle vier Listenansichten (Uebersicht, nach Kategorie, nach Klasse, Abhaengigkeitsgraph) und die beiden Lieferanten-Bildschirme (Formular, Selbstauskuenfte) waren fuer FR/NL durchgehend englisch — deutlich groesser als urspruenglich angenommen, da neben dem Formular fuenf weitere Funktionen betroffen waren. Rund 70 neue Schluessel in DE/EN/FR/NL, mehrere davon wiederverwendet (`assets_classification`, `assets_criticality`, `ui_notes`, `ui_cancel`, `ui_save`, `col_type`, `col_status`). Der Lieferanten-Statusbadge „Link kopieren" war zuvor unabhaengig von der gewaehlten Sprache fest auf Deutsch verdrahtet — jetzt ebenfalls ueber `t()`.
- **i18n-Nachzug, Teil 4: GDPR-Modul (VVT, AV, DSFA, Datenpannen, DSAR, TOMs, Loeschprotokoll, DSB).** Das gesamte GDPR-Modul war fuer FR/NL durchgehend englisch — Tab-Leiste, alle sieben Formulare/Listenansichten, sowie 12 Dropdown-Konstanten (Rechtsgrundlagen, Statuswerte, Vorfallstypen, DSAR-Typen, TOM-Kategorien, Art.-28-Checkliste). Rund 170 neue Schluessel in DE/EN/FR/NL, groesstes Einzelmodul bisher. Die 12 Konstanten wurden nach dem Getter-Muster umgestellt (siehe Governance-Commit) und nutzen bewusst vorhandene generische Schluessel (`legl_stDraft`, `govl_statApproved`, `riskl_*` u.a.) statt Duplikate.
- **i18n-Nachzug, Teil 3: Governance-Formulare.** Management-Review, Massnahmen und Sitzungsprotokolle (`openGovReviewForm`, `openGovActionForm`, `openGovMeetingForm`) waren fuer FR/NL durchgehend englisch. Jetzt uebersetzt — 47 neue Schluessel in DE/EN/FR/NL, ein guter Teil generisch und modulübergreifend wiederverwendbar (`ui_basicData`, `ui_notes`, `ui_owner`, `ui_dueDate`, `ui_participants`, `ui_docsAttachments` u.a.).
- **i18n-Nachzug, Teil 2: Legal-Formulare.** Die drei Legal-Formulare (Vertrag, NDA, Datenschutzrichtlinie) waren fuer FR/NL durchgehend englisch. Jetzt uebersetzt — 28 neue Schluessel in DE/EN/FR/NL, mehrere davon modulübergreifend wiederverwendet (`col_title`, `col_type`, `col_status`, `ui_cancel`, `ui_attachments`, `ui_ownerResp`, `ui_internalNotes`).
