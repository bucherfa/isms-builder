...
Enterprise environments commonly split infrastructure into separate network
segments (DMZ, application tier, database tier, ...) — sometimes still using
officially-routable (public) IP ranges internally, a legacy practice from
before RFC 1918 private ranges became universal. The questions below came up
while validating the #42 SQL-backend fix and are answered here so the
reasoning doesn't have to be re-derived each time.

| Aspect | Status | Why |
|---|---|---|
| DB connection across network segments/firewalls | **Not a concern** | The app connects to `DB_HOST:DB_PORT` over plain TCP (Knex + `mysql2`/`pg`). No code path inspects, whitelists, or assumes anything about the address range — verified by grep, no CIDR/private-range logic exists anywhere in `server/`. Any topology works as long as DNS resolves and routing/firewall rules allow the TCP connection. |
| Legacy segments using officially-routable IP ranges internally | **Not a concern** | Same reasoning — TCP/IP does not distinguish private vs. public address space; that's a routing decision entirely outside the application. |
| Encryption across segment boundaries | **Supported** | `DB_SSL=true` enables TLS for the MariaDB/PostgreSQL connection (see `.env.example`). |
| Actual multi-segment routing (app and DB on different subnets, traffic through a firewall) | **Not tested** | Verified so far only on a single flat network (Docker Compose default bridge). This is infrastructure/firewall configuration, not something application code needs to handle — no further code changes are implied by this gap. |
| `X-Forwarded-For`/`X-Forwarded-Host`/`X-Forwarded-Proto` behind a reverse proxy (typical when the app sits behind a DMZ-facing proxy) | **Fixed** | Found while investigating the questions above: the app read these headers directly and unconditionally, letting any client spoof the audit-trail IP on policy acknowledgements (`ackPublic.js`) and the host/protocol used to build acknowledgement-email links (`acknowledgements.js`), independent of network segmentation entirely. Fixed via Express's `trust proxy` setting, off by default (`TRUST_PROXY` env var to opt in explicitly when a reverse proxy is actually in front), plus an optional `PUBLIC_URL` override for the common case where the externally visible host differs from the one the app sees directly. See `tests/trustProxy.test.js`. |
