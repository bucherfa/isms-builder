...
**Aktueller Stand:** 326/326 Tests bestehen. Vor einem Push `npm run preflight` ausführen — das spiegelt exakt die harten CI-Schritte (Tests, pdf-parse-Pinning, `npm audit --audit-level=high`). Optionaler `pre-push`-Hook: `git config core.hooksPath .githooks`
